<div class="form-group">
    <div class="d-flex align-items-center mb-2">
        ...
        <a class="ml-3 btn btn-info btn-sm search-address-prefecture-postcode-btn"
            data-post-code-input="{{ $postCode['name'] }}" data-address-input="{{ $address['name'] }}"
            data-prefecture-select="{{ isset($prefecture) ? $prefecture['name'] : '' }}">{{ ___('mycustom.word.search_prefecture') }}</a>
    </div>
    <input type="text" name="{{ $postCode['name'] }}" class="form-control" id="{{ $postCode['name'] }}"
        value="{{ $postCode['value'] }}">
</div>
@if (isset($prefecture))
    @include('mycustom::components.form.select-prefecture', [
